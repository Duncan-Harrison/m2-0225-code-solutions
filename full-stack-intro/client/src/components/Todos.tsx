...
export function Todos() {
  ...
  useEffect(() => {
    async function fetchTodos(): Promise<any> {
      try {
        ...
        const foundTodo = (await response.json()) as Todo[];
        setTodos(foundTodo);
        return foundTodo;
      } catch (err) {
        console.error(err);
  ...
  async function addTodo(newTodo: UnsavedTodo) {
    try {
      ...
      const add = await fetch(`/api/todos`, request);
      if (!add.ok) throw new Error(`fetch request error ${add.status}`);
  ...
  async function toggleCompleted(todo: Todo) {
    ...
    try {
      ...
      const toggle = await fetch(`/api/todos/${todo.todoId}`, req);
      if (!toggle.ok) throw new Error(`Fetch error status ${toggle.status}`);
      const toComplete = (await toggle.json()) as Todo;
      const allTodos = todos.map((original) =>
        original.todoId === toComplete.todoId ? toComplete : original
      );
      setTodos(allTodos);
    } catch (err) {
      console.error(err);
