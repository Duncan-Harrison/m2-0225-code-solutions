...
export function UploadForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    try {
      ...
      const formula = event.currentTarget as HTMLFormElement;
      const newForm = new FormData(formula);
      console.log(newForm);
      const request = new Request('/api/uploads', {
        method: 'POST',
        body: newForm,
      });
  ...
  return (
    <div>
      ...
      {imageFile && <img src={imageFile.url} alt={imageFile.url} width="240" />}
    </div>
  );
}
